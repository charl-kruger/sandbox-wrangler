...
function REPL() {
	...
	useEffect(() => {
		const httpClient = new HttpClient({
			...
			onCommandComplete: (success: boolean, exitCode: number, stdout: string, stderr: string, command: string, args: string[]) => {
				setResults((prev) => {
					const updated = [...prev];
	...
	const executeCommand = async () => {
		...
		try {
			...
			const result = await client.execute(command, args, sessionId || undefined);
		...
		<div className="repl-container">
			...
			<div className="command-bar">
				...
				<div className="action-buttons">
					<button type="button" onClick={executeCommand} disabled={!commandInput.trim() || isExecuting} className="btn btn-execute">
						...
						{results.map((result) => (
							<div key={result.id} className="command-result">
								<div className="result-header">
									...
									{result.status !== 'running' && result.exitCode !== undefined && (
			...
			<div className="help-section">
				...
				<div className="help-note">
					<strong>Note:</strong> Use the "Stream" button for commands that produce real-time output (like <code>top</code> or{' '}
					<code>tail -f</code>).
				</div>
			</div>
		</div>
